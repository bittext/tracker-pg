...
Without `TRACKER_UPSERT_PASSWORD`, you can put **role first** when the next token is `true` or `false` (or role is the only extra token), e.g. `bash scripts/add-user.sh demo USER true false` — you are then prompted for the password.
...
**Management → Account** (passwords, authenticator keys, websites, notes) are stored per signed-in owner in the `management_accounts` table. The `password` and `authenticator_key` columns are sealed with **AES-256-GCM** when **`TRACKER_MANAGEMENT_ACCOUNTS_ENCRYPTION_KEY`** is set (long random passphrase or Base64-encoded 32-byte key); when unset, they are stored as plaintext (development only). On first login after upgrading to this build, the web app automatically uploads any legacy entries from browser **`localStorage`** (previously labelled "Utilities") to the server vault and clears the local copies. Rotating the key invalidates existing sealed rows — back up the database before changing it. The web **Privacy policy** includes a **Financial data & Plaid** section; members must record acknowledgment (**`POST /api/me/privacy/plaid-financial-data-notice`**, also invoked from Finance → Banking before Link) before **`link-token`** or **`exchange`** succeed. **`DELETE /api/finance/banking/plaid/link?institutionId=`** removes stored Plaid credentials for that institution (imported transactions/files stay until removed separately). Authenticated API flow:
...
**Deploy script (`scripts/lightsail-deploy.sh`):** after `git pull`, run `bash scripts/lightsail-deploy.sh` to rebuild **api** and **web** (and start **caddy** when Caddy is enabled, **robinhood-notebook** when notebook render is enabled) without recreating **postgres**. To always merge **`docker-compose.robinhood.yml`** on that host, either run `export TRACKER_ROBINHOOD_COMPOSE=1` or once: `touch .use-lightsail-robinhood-compose` in the repo root (gitignored). For **Caddy/HTTPS**, use **`TRACKER_CADDY=1`** or **`touch .use-caddy-lightsail`**. For **notebook render**, set **`TRACKER_FINANCE_ROBINHOOD_NOTEBOOK_SERVICE_ENABLED=true`** in `.env.stack` or **`touch .use-lightsail-robinhood-notebook`**. In GitHub Actions, set **`LIGHTSAIL_USE_ROBINHOOD_COMPOSE`**, **`LIGHTSAIL_USE_CADDY`**, and/or **`LIGHTSAIL_USE_ROBINHOOD_NOTEBOOK`** to **`1`** as needed.
...
- Exercise
- Finance
- Management (tasks; Work tab with work-category tasks, daily work log with file attachments and inline audio playback; Travel tab with MapLibre map, trips/places, optional place photos, and address search via OpenStreetMap Nominatim; calendar; account vault; month notes; write-ups)
