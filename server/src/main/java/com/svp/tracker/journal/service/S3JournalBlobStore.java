...
public class S3JournalBlobStore implements JournalBlobStore, DisposableBean {
    ...
    @Nullable
    private final ObjectCannedACL putAcl;

    public S3JournalBlobStore(String bucket, S3Client client) {
        this(bucket, client, null);
    }

    public S3JournalBlobStore(String bucket, S3Client client, @Nullable ObjectCannedACL putAcl) {
        this.bucket = bucket;
        this.client = client;
        this.putAcl = putAcl;
    }
    ...
    public String put(long ownerUserId, long entryId, InputStream in, long sizeBytes) throws IOException {
        ...
        try (InputStream input = in) {
            ...
            try {
                ...
                client.putObject(put.build(), RequestBody.fromBytes(payload));
                ...
                if (redirectRegion != null) {
                    try (S3Client regional = clientForRegion(redirectRegion)) {
                        ...
                        regional.putObject(put.build(), RequestBody.fromBytes(payload));
                        return key;
