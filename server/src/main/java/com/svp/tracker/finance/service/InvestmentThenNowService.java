...
public class InvestmentThenNowService {
    ...
    public InvestmentThenNowResultDto compute(InvestmentThenNowRequestDto body) {
        ...
        ChartSnapshot chart = fetchChart(symbol, asOf.minusDays(14), today.plusDays(1));
        ...
        row.setPriceSource("yahoo-chart");
    ...
    private ChartSnapshot fetchChart(String symbol, LocalDate start, LocalDate end) {
        try {
            ...
            if (resp.statusCode() < 200 || resp.statusCode() >= 300) {
                throw new ResponseStatusException(
                        HttpStatus.BAD_GATEWAY, "Yahoo chart HTTP " + resp.statusCode());
            }
            return parseChart(objectMapper.readTree(resp.body()));
        } catch (ResponseStatusException e) {
            throw e;
        } catch (Exception e) {
            log.warn("then/now price fetch failed for {}", symbol, e);
            throw new ResponseStatusException(
                    HttpStatus.BAD_GATEWAY, "Could not load price history for " + symbol + ": " + e.getMessage());
        }
    }
    ...
    private static ChartSnapshot parseChart(JsonNode root) {
        ...
        if (!results.isArray() || results.isEmpty()) {
            return new ChartSnapshot("", null, bars);
        }
        ...
        return new ChartSnapshot(company, live, bars);
    }
    ...
    private record ChartSnapshot(String companyName, Double regularMarketPrice, NavigableMap<LocalDate, Double> bars) {}
}
