...
import {
  ...
  activeExerciseIsos,
  buildDurationBars,
  buildMonthWeightLbByDay,
  buildPersistenceStory,
  buildWeightTrendSummary,
  computeStreakStats,
  sumDurationMinutes,
} from '../../../util/exercise-report-trends.util';
...
@Component({
  selector: 'app-reports-exercise',
  standalone: true,
  imports: [CommonModule, MatCardModule, MatTableModule, MatSnackBarModule, MatButtonModule, MatIconModule],
  templateUrl: './reports-exercise.component.html',
  styleUrl: './reports-exercise.component.scss',
})
export class ReportsExerciseComponent implements OnInit {
  ...
  readonly monthCal = signal<MonthActivityCalendarDto | null>(null);
  readonly monthlyReport = signal<MonthlyExerciseReportDto | null>(null);
  dailyReport: DailyExerciseReportDto | null = null;
  dayDetailError: string | null = null;

  /** Full weight history for trend charts (newest first from API). */
  private readonly bodyWeightLogs = signal<BodyWeightLog[]>([]);
  ...
  readonly weightTrend = computed(() => {
    ...
    const byDay = buildMonthWeightLbByDay(y, m, this.bodyWeightLogs(), cal?.bodyWeightKgByDay);
    return buildWeightTrendSummary(byDay);
  });

  readonly durationBars = computed(() =>
    buildDurationBars(this.calendarYear(), this.calendarMonth(), this.monthCal()?.exerciseDurationMinutesByDay),
  );

  readonly streakStats = computed(() =>
    computeStreakStats(
      activeExerciseIsos(this.monthCal()),
      this.calendarYear(),
      this.calendarMonth(),
      this.todayIso(),
    ),
  ...
  readonly persistenceStory = computed(() => {
    const trend = this.weightTrend();
    const streaks = this.streakStats();
    const cal = this.monthCal();
    const exerciseDays = this.monthlyReport()?.workoutDays ?? activeExerciseIsos(cal).length;
    return buildPersistenceStory({
      year: this.calendarYear(),
      month: this.calendarMonth(),
      todayIso: this.todayIso(),
      exerciseDays,
      activeIsos: activeExerciseIsos(cal),
      weighInCount: trend.weighInCount,
      weightDeltaLb: trend.deltaLb,
      currentStreak: streaks.currentStreak,
      bestStreakMonth: streaks.bestStreakMonth,
    });
  });

  readonly hasTrendData = computed(() => {
    const exerciseDays = this.monthlyReport()?.workoutDays ?? 0;
    return exerciseDays > 0 || this.weightTrend().weighInCount > 0;
  });

  ngOnInit(): void {
    const t = this.todayIso();
    this.selectedDateIso = t;
    this.calendarYear.set(Number(t.slice(0, 4)));
    this.calendarMonth.set(Number(t.slice(5, 7)));
    this.loadBodyWeightHistory();
  ...
  formatWeightDelta(deltaLb: number | null): string {
    if (deltaLb == null) {
      return '—';
    }
    if (Math.abs(deltaLb) < 0.05) {
      return '0 lb';
    }
    const sign = deltaLb > 0 ? '+' : '−';
    return `${sign}${Math.abs(deltaLb).toFixed(1)} lb`;
  }

  monthExerciseMinutes(): number {
    return sumDurationMinutes(this.monthCal()?.exerciseDurationMinutesByDay);
  }

  selectedDayExerciseMinutes(): number {
    const lines = this.dailyReport?.logLines ?? [];
    return lines.reduce((sum: number, row: DailyExerciseLogLineDto) => {
      const n = Number(row.durationMinutes ?? 0);
      return Number.isFinite(n) && n > 0 ? sum + Math.floor(n) : sum;
    }, 0);
  }

  /** Delta vs previous weigh-in before the selected day (from full history). */
  selectedDayWeightDeltaLb(): number | null {
    const iso = this.selectedDateIso;
    if (!iso || this.dailyReport?.bodyWeightKg == null) {
      return null;
    }
    const currentLb = this.kgToLb(this.dailyReport.bodyWeightKg);
    const earlier = this.bodyWeightLogs()
      .map((l) => ({ on: l.loggedOn?.slice(0, 10) ?? '', lb: l.weightLb ?? (l.weightKg != null ? this.kgToLb(l.weightKg) : null) }))
      .filter((l) => l.on && l.on < iso && l.lb != null)
      .sort((a, b) => b.on.localeCompare(a.on));
    if (!earlier.length || earlier[0].lb == null) {
      return null;
    }
    return currentLb - earlier[0].lb;
  }
  ...
  private loadBodyWeightHistory(): void {
    this.fitnessApi.listBodyWeight().subscribe({
      next: (rows) => this.bodyWeightLogs.set(rows ?? []),
      error: () => this.bodyWeightLogs.set([]),
    });
  }
  ...
  private loadExerciseMonth(): void {
    ...
    forkJoin({
      ...
      next: ({ monthly, cal }) => {
        this.monthlyReport.set(monthly);
        this.monthCal.set(cal);
      },
